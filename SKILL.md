...
8. **直接产出富文本 HTML**：官网 CMS 富文本编辑器只吃 HTML，不吃 Markdown。写作提示词第十四节要求模型**直接以富文本 HTML 输出正文**（不产出 Markdown）；长尾词 + SEO 信息放顶部 HTML 注释。最终只交付一个 `.html`，直接粘进 CMS 正文框发布。若偶发产出 Markdown，用第 6 步 `md2html.py` 兜底转换。
